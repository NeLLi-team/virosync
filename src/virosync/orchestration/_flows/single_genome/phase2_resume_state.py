...
def _require_float_list(value: object, context: str) -> list[float]:
    if not isinstance(value, list):
        raise Phase2ResumeStateError(f"{context} must be a list")
    return [_require_finite_float(item, f"{context}[{index}]") for index, item in enumerate(value)]
...
def _validate_model_fields() -> None:
    ...
    for model, expected_fields in expected:
        ...
        if actual_fields != expected_fields:
            raise Phase2ResumeStateError(
                f"{model.__name__} fields changed without a Phase-2 resume state schema update"
            )
...
def _require_exact_instance(value: object, model: type, context: str) -> None:
    ...
    if set(vars(value)) != expected:
        raise Phase2ResumeStateError(f"{context} contains unsupported dynamic fields")
...
def _mapping_from_entries(
    ...
    for index, raw_entry in enumerate(value):
        ...
        if key in result:
            raise Phase2ResumeStateError(f"{context} contains duplicate key {key!r}")
...
def _gene_to_document(value: object, context: str) -> dict[str, object]:
    ...
    for name in _GENE_STRING_FIELDS:
        document[name] = _require_string(getattr(value, name), f"{context}.{name}")
    for name in _GENE_INTEGER_FIELDS:
        document[name] = _require_integer(getattr(value, name), f"{context}.{name}")
    for name in _GENE_FLOAT_FIELDS:
        document[name] = _require_finite_float(getattr(value, name), f"{context}.{name}")
    for name in _GENE_STRING_LIST_FIELDS:
        document[name] = _require_string_list(getattr(value, name), f"{context}.{name}")
    for name in _GENE_FLOAT_LIST_FIELDS:
        document[name] = _require_float_list(getattr(value, name), f"{context}.{name}")
    for name in _GENE_BOOLEAN_FIELDS:
        document[name] = _require_boolean(getattr(value, name), f"{context}.{name}")
...
def phase2_resume_state_to_document(
    ...
    if not isinstance(state.refined_boundaries, Sequence) or isinstance(state.refined_boundaries, (str, bytes)):
    ...
    return {
        ...
        "refined_boundaries": phase2_state_to_document(state.refined_boundaries),
...
def phase2_resume_state_from_document(document: object) -> Phase2ResumeState:
    ...
    if not valid_schema_version:
        raise Phase2ResumeStateError(f"unsupported Phase-2 resume schema_version: {schema_version!r}")
    artifact_type = _require_string(payload["artifact_type"], "phase2 resume state.artifact_type")
    if artifact_type != PHASE2_RESUME_STATE_ARTIFACT_TYPE:
        raise Phase2ResumeStateError(f"unsupported Phase-2 resume artifact_type: {artifact_type!r}")
    ...
    return Phase2ResumeState(
        refined_boundaries=phase2_state_from_document(payload["refined_boundaries"]),
...
def _object_without_duplicate_keys(
    ...
    for key, value in pairs:
        if key in result:
            raise Phase2ResumeStateError(f"duplicate JSON key in Phase-2 resume state: {key!r}")
        result[key] = value
    return result


def _reject_nonstandard_constant(value: str) -> object:
    raise Phase2ResumeStateError(f"non-finite JSON number in Phase-2 resume state: {value}")
...
def write_phase2_resume_state(
    ...
    *,
    ...
    try:
        ...
        raise Phase2ResumeStateError(f"Phase-2 resume state is not JSON-safe: {exc}") from exc
...
def load_phase2_resume_state(path: str | Path) -> Phase2ResumeState:
    ...
    try:
        ...
        raise Phase2ResumeStateError(f"invalid Phase-2 resume state {state_path}: {exc}") from exc
