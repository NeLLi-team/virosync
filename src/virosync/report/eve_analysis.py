...
#
# ViroSync reports candidate endogenous viral elements (EVEs), with
# flanking genes, gene taxonomy, MCP support, and fold classifications.
#
# Confidence is a rule-based score, not a calibrated EVE probability. The
# published taxonomy can come from an MCP override or an ANI-connected
# donor. A donor need not have a direct ANI edge to every recipient. Inspect
# the gene evidence and edge table when a region has conflicting signals.
#
...
# ## ANI Network of EVEs
#
# One node represents one EVE. Graphviz `sfdp` sets node positions. An edge
# means that a reported skani comparison reaches `ANI_THRESHOLD` and covers
# at least `MIN_ALIGNED_FRACTION` of either sequence. The data come from
# `phase3_synthesis/eve_ani_edges.tsv`. Node fill shows the published taxonomy;
# a thick black border marks an eligible MCP class donor. Only nodes with an
# edge appear, and the title counts omitted nodes. Missing edges do not prove
# that regions are unrelated: short sequences can lack enough sketch matches.
# This plot does not recompute clusters. At the default thresholds, its
# connected components match the multi-member pipeline clusters.
...
#
# Each canonical EVE includes every Prodigal gene in its displayed span.
# Gray flanks cover `FLANK_SIZE` (5 kb by default) and extend to include the
# three nearest genes on each side. Arrows follow the Prodigal strand. Gene
# colors use the highest-ranked viral top-10 hit with at least 25% amino-acid
# identity, or the best protein hit if none qualifies. Eukaryotic hits that
# share the dominant Phase 1 host lineage have a separate color.
# Gold circles mark supported MCPs. White stars show DJR, SJR, or HK97 fold
# classifications. Fold labels can use heuristic or domain evidence and do
# not establish MCP support on their own. Legacy PHAGE targets with a resolved
# Preplasmiviricota lineage appear as PPV. The white interval marks the final
# ViroSync boundary.
