...
def graphviz_runtime_error(*, timeout_seconds: float = 30) -> str | None:
    ...
    attributes = ", ".join(f'{name}="{value}"' for name, value in ANI_NETWORK_RENDER_ATTR.items())
    ...
    if result.returncode != 0 or not result.stdout.startswith(_PNG_SIGNATURE):
        ...
        return "Graphviz cannot render the required sfdp PNG report" + (f": {detail}" if detail else "")
    return None
